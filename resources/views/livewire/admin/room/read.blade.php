            ...
            <div class="card-header p-0">
                ...
                <div class="px-2 mt-4">
                    ...
                    <div class="row justify-content-between mt-4 mb-4">
                        @if(getCrudConfig('Room')->create && hasPermission(getRouteName().'.room.create', 0, 0))
                        <div class="col-md-4 right-0">
                            <a href="@route(getRouteName().'.room.create')" class="btn btn-success">{{ __('CreateTitle', ['name' => __('Room') ]) }}</a>
                        </div>
                        @endif
                        @if(getCrudConfig('Room')->searchable())
                        <div class="col-md-4">
                ...
                <table class="table table-hover table-striped">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" style='cursor: pointer' wire:click="sort('room_type')"> <i class='fa @if($sortType == 'desc' and $sortColumn == 'room_type') fa-sort-amount-down ml-2 @elseif($sortType == 'asc' and $sortColumn == 'room_type') fa-sort-amount-up ml-2 @endif'></i> {{ __('Room_type') }} </th>
                    ...
                    <tbody>
                        @foreach($rooms as $room)
                            @livewire('admin.room.single', [$room], key($room->id))
                        @endforeach
                    </tbody>
                </table>
            </div>
